...
function c_sec_safe($str)
{
	...
	if (preg_match('/(<\s*SCRIPT|SCRIPT\s*>)/i', $decode))
	{
		return;
	}
	if (preg_match('/(<\s*IFRAME|IFRAME\s*>)/i', $decode))
	{
		return;
	}
	if (preg_match('/(UNION|SELECT|CONCAT|DELETE|INSERT|DROP|FROM|WHERE) /i', $decode))
	{
		return;
	}
	if (preg_match('/(UNION|SELECT|CONCAT|DELETE|INSERT|DROP|FROM|WHERE)\(/i', $decode))
