...
// 30 jan 2020 by joh ** check auth privacy access leader only//
require_once "function.inc.php";
authPrivatePageLeader();
// end //
...
<head>
	...
	<link rel="stylesheet" href="../../css/tabletab_leader_index.css"/>
...
<div>
	...
	<div class="container col-sm-8"><br>
		<div class="row">
		<h3>แจ้งซ่อมใหม่&nbsp;&nbsp;&nbsp;</h3>
		<a href="../../PHPExcel/export-excel-leader1.php" class="btn btn-outline-success" target="_blank">excel</a>
	</div><br></div>
	...
	<div class="container-fluid">
		<ul class="nav nav-tabs">
			...
			<li class="nav-item" style="display:inline-block;">
				<a class="nav-link active" href="leader_index.php"><img src="../../img/icon/status1.png" width="15px" height="15px"><span class="badge badge-danger"><?php echo $count_status1; ?></span></a>
			</li>
			<li class="nav-item" style="display:inline-block;">
				<a class="nav-link" href="leader_index_status2.php"><img src="../../img/icon/status2.png" width="15px" height="15px"><span class="badge badge-danger"><?php echo $count_status2; ?></span></a>
			</li>
			<li class="nav-item" style="display:inline-block;">
	...
	<div class="divtable accordion-xs">
		...
		$query = "SELECT COUNT(*) as 'num' FROM MaintenanceRepairAdd WHERE MRA_Status = 1";
		$result = sqlsrv_query($conn, $query) or die(print_r(sqlsrv_errors(), true));
		...
		// $_GET['page'] == 1 ไม่เข้าเงื่อนไขเปลี่ยนเป็น $_GET['page'] == '' แทน 16012020-by joh
		...
		// add sort MRA_Datetime DESC 16012020 -by joh
		$sql = "SELECT * FROM MaintenanceRepairAdd WHERE MRA_Status = 1 ORDER BY MRA_Priority DESC, MRA_Datetime DESC OFFSET " . $s_page . " ROWS FETCH NEXT " . $e_page . " ROWS ONLY";
		...
		if (sqlsrv_has_rows($result) != 0)
		{
			...
			while ($row = sqlsrv_fetch_array($result))
			{
				switch ($row["MRA_Status"])	 {
					// case "1":		
					//  $status = '<span class="badge badge-pill badge-danger">แจ้งซ่อมใหม่</span>'; "";				
					// 	$status_number = 1;				
					// 	break;
					case "2":		
						$status = '<span class="badge badge-pill badge-info">กำลังดำเนินการ</span>'; "";			
						$status_number = 2;				
						break;
					case "3":		
						$status = '<span class="badge badge-pill badge-warning">ดำเนินการเสร็จ / รอตรวจงาน</span>'; "";	
						$status_number = 3;				
						break;
					case "4":		
						$status = '<span class="badge badge-pill badge-primary">รอปิดงาน</span>'; "";					
						$status_number = 4;				
						break;
					case "5":		
						$status = '<span class="badge badge-pill badge-success">ปิดงานแล้ว</span>'; "";					
						$status_number = 5;				
						break;
					default : 
						$status = '<span class="badge badge-pill badge-success">แจ้งซ่อมใหม่</span>'; "";					
						$status_number = 1;				
						break;
				}
				...
				echo '<div class="td worktype">' . $classMySec->encode($row["MRA_WorkType"]) . '</div>';
				echo '<div class="td priority">' . $row["MRA_Priority"] . '</div>';
				echo '<div class="td description">' . $classMySec->encode($row["MRA_Description"]) . '</div>';
				echo '<div class="td effect" id="limittext_layout">' . $classMySec->encode($row["MRA_Effect"]) . '</div>';
				...
				echo '<div class="td manage"><a href="leader_assign.php?id=' . $row["MRA_ID"] . '" class="btn btn-light-blue btn-sm">มอบหมายงาน</a></div>';
				echo "</div>";
				echo "</div>";
				echo "</div>";
				$num++;
		...
		// echo "total => ". $total."<br>";
		// echo "cur_page => ". $_GET['page']."<br>";
		// echo "e_page => ". $e_page."<br>";
		// echo "GET => "; print_r($_GET);

		?>
		<div style="float: right; margin: 20px;"><?php page_navi($total, (isset($_GET['page'])) ? $_GET['page'] : 1, $e_page, 10); ?></div>
	</div>
