...
if (isset($_SESSION['EP_ID']) && in_array($_SESSION['EP_ID'], $authEPID)) {
    ...
    if(isset($_SESSION["WG"]) || isset($_GET['workgroup'])){
        ...
        $query1 = "SELECT COUNT(*) as 'num1' FROM MaintenanceRepairAdd 
        ...
        WHERE Section.S_ID = '$department' AND MaintenanceRepairAdd.MRA_Status = 1";
        $result1 = sqlsrv_query($conn, $query1) or die(print_r(sqlsrv_errors(), true));

        $query2 = "SELECT COUNT(*) as 'num2' FROM MaintenanceRepairAdd 
        INNER JOIN WorkGroup  ON MaintenanceRepairAdd.MRA_WorkUnitID = WorkGroup.WG_ID
        INNER JOIN Department  ON WorkGroup.D_ID = Department.D_ID
        INNER JOIN Section  ON Department.S_ID = Section.S_ID 
        WHERE Section.S_ID = '$department' AND MaintenanceRepairAdd.MRA_Status = 2";
        $result2 = sqlsrv_query($conn, $query2) or die(print_r(sqlsrv_errors(), true));

        $query3 = "SELECT COUNT(*) as 'num3' FROM MaintenanceRepairAdd 
        INNER JOIN WorkGroup  ON MaintenanceRepairAdd.MRA_WorkUnitID = WorkGroup.WG_ID
        INNER JOIN Department  ON WorkGroup.D_ID = Department.D_ID
        INNER JOIN Section  ON Department.S_ID = Section.S_ID 
        WHERE Section.S_ID = '$department' AND MaintenanceRepairAdd.MRA_Status = 3";
        $result3 = sqlsrv_query($conn, $query3) or die(print_r(sqlsrv_errors(), true));

        $query4 = "SELECT COUNT(*) as 'num4' FROM MaintenanceRepairAdd 
        INNER JOIN WorkGroup  ON MaintenanceRepairAdd.MRA_WorkUnitID = WorkGroup.WG_ID
        INNER JOIN Department  ON WorkGroup.D_ID = Department.D_ID
        INNER JOIN Section  ON Department.S_ID = Section.S_ID 
        WHERE Section.S_ID = '$department' AND MaintenanceRepairAdd.MRA_Status = 4";
        $result4 = sqlsrv_query($conn, $query4) or die(print_r(sqlsrv_errors(), true));

        $query5 = "SELECT COUNT(*) as 'num5' FROM MaintenanceRepairAdd 
        INNER JOIN WorkGroup  ON MaintenanceRepairAdd.MRA_WorkUnitID = WorkGroup.WG_ID
        INNER JOIN Department  ON WorkGroup.D_ID = Department.D_ID
        INNER JOIN Section  ON Department.S_ID = Section.S_ID 
