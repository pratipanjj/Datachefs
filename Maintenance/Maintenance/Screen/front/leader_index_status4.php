...
<div>
	...
	<div class="container col-sm-8"><br>
		<div class="row">
		<h3>รอปิดงาน&nbsp;&nbsp;&nbsp;</h3>
		<a href="../../PHPExcel/export-excel-leader4.php" class="btn btn-outline-success" target="_blank">excel</a>
	</div><br></div>
	...
	<div class="container-fluid">
		<ul class="nav nav-tabs">
			...
			<li class="nav-item" style="display:inline-block;">
				<a class="nav-link active" href="leader_index_status4.php"><img src="../../img/icon/status4.png" width="15px" height="15px"><span class="badge badge-danger"><?php echo $count_status4; ?></span></a>
			</li>
			<li class="nav-item" style="display:inline-block;">
	...
	<div class="divtable accordion-xs">
		...
		$query = "SELECT COUNT(*) as 'num' FROM MaintenanceRepairAdd WHERE MRA_Status = 4";
		$result = sqlsrv_query($conn, $query) or die(print_r(sqlsrv_errors(), true));
		...
		$sql = "SELECT * FROM MaintenanceRepairAdd WHERE MRA_Status = 4 ORDER BY MRA_Priority DESC OFFSET " . $s_page . " ROWS FETCH NEXT " . $e_page . " ROWS ONLY";
		...
		if (sqlsrv_has_rows($result) != 0)
		{
			...
			while ($row = sqlsrv_fetch_array($result))
			{
				...
				echo '<div class="td manage"><a href="leader_closejob.php?id=' . $row["MRA_ID"] . '" class="btn btn-light-blue btn-sm">ปิดงาน</a></div>';
				echo "</div>";
				echo "</div>";
				echo "</div>";
				$num++;
