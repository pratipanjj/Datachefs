...
<head>
	...
	<link rel="stylesheet" href="../../css/tabletab_leader_repairlist.css"/>
...
<!-- <nav class="navbar navbar-expand-lg navbar-light bg-light">
	...
	<div class="collapse navbar-collapse" id="navbarTogglerDemo01">
		...
		<ul class="navbar-nav mr-auto mt-2 mt-lg-0">
			...
			<li class="nav-item">
				<a class="nav-link active" href="leader_repair_list.php">รายการแจ้งซ่อม</a>
			</li>
			<li class="nav-item">
...
<div class="divtable accordion-xs">
	...
	$sql = "SELECT * FROM MaintenanceRepairAdd ORDER BY MRA_ID DESC OFFSET " . $s_page . " ROWS FETCH NEXT " . $e_page . " ROWS ONLY";
	$results = sqlsrv_query($conn, $sql);


	if (sqlsrv_has_rows($results) != 0)
	{
		...
		while ($row = sqlsrv_fetch_array($results))
		{
			...
			echo '<div class="td description" id="limittext_layout">' . $classMySec->encode($row["MRA_Description"]) . '</div>';
			echo '<div class="td effect" id="limittext_layout">' . $row["MRA_Effect"] . '</div>';
			...
			if ($row["MRA_Status"] != 1)
			{
				echo '<div class="td manage"><button href="#" class="btn btn-light-blue btn-sm" disabled>มอบหมายงาน</button></div>';
			}
			else
			{
				echo '<div class="td manage">
						<a href="leader_assign.php?id=' . $row["MRA_ID"] . '" class="btn btn-light-blue btn-sm">มอบหมายงาน</a> &nbsp;
					</div>';
			}

			// -------------------------------------20/01/63 ปิดปุ่มลบรายการ by POP-------------------------------------------------------------
			// <a href="javascript:void(0);" class="btn btn-outline-danger btn-sm btn-delete" id="'.$row["MRA_ID"].'">ลบรายการ</a>
			// ------------------------------------------------------------------------------------------------------------------------------
...
<script type="text/javascript">
	...
	$('.btn-delete').click(function() { 
		...
		$.ajax({
			/*
				remove ip address by joh
			 */
			// url: 'http://203.151.212.56/Maintenance/Screen/backend/action_leader_delete.php',
			url: '<?=PATH_DEV?>/Maintenance/Screen/backend/action_leader_delete.php',
			type: 'GET',
			dataType: 'TEXT',
			data: { id : value_id },
		})
