...
<div>
	...
	<div class="container col-sm-8"><br>
		...
		<a href="../../PHPExcel/export-excel-leader5.php" class="btn btn-outline-success" target="_blank">excel</a>
	</div><br></div>
	...
	<div class="container-fluid">
		<ul class="nav nav-tabs">
			...
			<li class="nav-item" style="display:inline-block;">
				<a class="nav-link active" href="leader_index_status5.php"><img src="../../img/icon/status5.png" width="15px" height="15px"><span class="badge badge-danger"><?php echo $count_status5; ?></span></a>
			</li>
			<hr>
		</ul>
	</div>
	...
	<div class="divtable accordion-xs">
		...
		$query = "SELECT COUNT(*) as 'num' FROM MaintenanceRepairAdd WHERE MRA_Status = 5";
		$result = sqlsrv_query($conn, $query) or die(print_r(sqlsrv_errors(), true));
		...
		$sql = "SELECT * FROM MaintenanceRepairAdd WHERE MRA_Status = 5 ORDER BY MRA_Priority DESC OFFSET " . $s_page . " ROWS FETCH NEXT " . $e_page . " ROWS ONLY";
		...
		if (sqlsrv_has_rows($result) != 0)
		{
			...
			while ($row = sqlsrv_fetch_array($result))
			{
				...
				echo '<div class="td pdf"><button type="submit" name="PDFFinish[]" id="PDFFinish_' . $WorkId . '" class="btn btn-danger btn-sm PDFFinish" data-id="' . $WorkId . '"><i class="far fa-file-excel"></i>PDF</button></div>';
				echo "</div>";
				echo "</div>";
				echo "</div>";
				$num++;
...
<script>
	...
	$(function (){
		...
		$('.PDFFinish').on('click', function() {
			let id = $(this).data('id');
			window.open("ReportPDF.php?ID="+ id);
			// console.log(id);
  		})
	});
</script>
